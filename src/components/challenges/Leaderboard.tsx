...
import { FriendProgress, Challenge } from "../../types/challenges.type";
...
export const Leaderboard: React.FC<LeaderboardProps> = ({
  ...
  const allPlayers = [...combinedFriends, mePlayer].sort(
  ...
  const getRank = (currentDay: number) => {
    ...
    const earnedRank = challenge.ranks
      .slice()
      .reverse()
      .find((r) => percent >= r.threshold);
          ...
          return (
            ...
            >
              ...
              <div className="relative shrink-0">
                ...
                />
                {index === 0 && (
                  <div className="absolute -top-3 -right-1 text-lg animate-bounce">
                    👑
                  </div>
                )}
              </div>
              ...
              <div className="flex-grow min-w-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[11px] font-bold uppercase tracking-wider ${rankInfo.color}`}
                  >
                    {rankInfo.name}
                  </span>
                </div>
                <div className="w-full bg-gray-100 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-1000 ${
                      isMe ? "bg-emerald-500" : "bg-gray-300"
                    }`}
                    style={{
                      width: `${Math.min(
                        (player.progress / challenge.duration) * 100,
                        100
                      )}%`,
                    }}
                  ></div>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm rounded-3xl animate-[fade-in_0.2s]">
          <div className="text-center p-6 w-full">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Challenge your friends to see who can reach the top rank first!
            </p>
