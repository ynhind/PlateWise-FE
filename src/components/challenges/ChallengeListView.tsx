...
export const ChallengeListView: React.FC<ChallengeListViewProps> = ({
    ...
    const currentList = activeTab === "active" ? activeChallenges : communityChallenges;
    ...
    const handleLeave = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        if (window.confirm("Are you sure you want to quit this challenge? All progress will be lost.")) {
            leaveChallenge(id);
        }
    };
    ...
    return (
        <div className="animate-[fade-in_0.5s_ease-out]">
            ...
            <div className="flex gap-8 mb-8 border-b border-gray-200">
                ...
                >
                    My Active <span className="text-sm ml-1 py-0.5 px-2 bg-gray-100 rounded-full text-gray-600">{activeChallenges.length}</span>
                </button>
                <button
                ...
                >
                    Community <span className="text-sm ml-1 py-0.5 px-2 bg-gray-100 rounded-full text-gray-600">{communityChallenges.length}</span>
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {displayedChallenges.map((challenge) => {
                    const progressPercent = calculateProgress(challenge.id, challenge.duration);
                    const completedDays = getLogsForChallenge(challenge.id).length;
                    
                    return (
                        <div
                            key={challenge.id}
                            onClick={() => activeTab === "active" ? onChallengeClick(challenge.id) : null}
                            ...
                            <div className="h-48 bg-gray-100 overflow-hidden relative">
                                ...
                                <div className="absolute top-3 left-3 flex gap-1 z-10">
                                    {challenge.tags && challenge.tags.slice(0, 2).map(tag => (
                                        <span key={tag} className="bg-yellow-400 text-yellow-950 text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                                            {tag}
                                        </span>
                                    ))}
                            ...
                            <div className="p-6 flex flex-col flex-grow">
                                ...
                                <div className="flex flex-wrap gap-1 mb-4">
                                    {challenge.tags?.map(tag => (
                                        <span key={tag} className="text-xs text-gray-500 bg-gray-50 border border-gray-100 px-2 py-0.5 rounded">#{tag}</span>
                                    ))}
                                </div>
                                    ...
                                    {activeTab === "active" ? (
                                        <div>
                                            ...
                                            <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                                <div 
                                                    className={`h-2 rounded-full transition-all duration-1000 ${progressPercent === 100 ? 'bg-green-500' : 'bg-emerald-500'}`} 
                                                    style={{ width: `${progressPercent}%` }}
                                                ></div>
                                            </div>
                                            {progressPercent === 100 && (
                ...
                {displayedChallenges.length === 0 && (
                    <div className="col-span-full text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
                        <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4">
                             <svg className="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        </div>
                        <h3 className="text-xl font-bold text-gray-900 mb-2">No challenges found</h3>
                        <p className="text-gray-500 max-w-md mx-auto">
                            We couldn't find any {selectedTag !== 'All' ? `"${selectedTag}"` : ''} challenges. 
                            {activeTab === 'active' ? ' Join one from the Community!' : ' Try adjusting your filters.'}
                        </p>
