...
interface ChallengeContextType {
    ...
    createChallenge: (challenge: Omit<Challenge, 'id' | 'status' | 'startDate'>) => void;
...
export const ChallengeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        localStorage.setItem('platewise_active_challenges_v2', JSON.stringify(activeChallenges));
    }, [activeChallenges]);

    useEffect(() => {
        localStorage.setItem('platewise_community_challenges_v2', JSON.stringify(communityChallenges));
    }, [communityChallenges]);
    ...
    const createChallenge = (data: Omit<Challenge, 'id' | 'status' | 'startDate'>) => {
